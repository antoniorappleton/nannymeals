...
export function parseContinente($: cheerio.CheerioAPI, url: string = ''): ScrapedRecipe {
  ...
  // Robust prepTime and servings detection with LD+JSON fallback
  const ldJson = parseLdJson($.html());
  let prepTime = ldJson?.prepTime || 0;
  let servings = ldJson?.servings || 4;
...
export function parseAuchan(
  ...
  const name =
    $("h1.recipe-hero__title").text().trim() || 
  ...
  const ingredients: ScrapedIngredient[] = [];
  $(".recipe-ingredients__item, .ingredients__item").each((_: number, el: any) => {
    const text = $(el).text().trim().replace(/\s+/g, " ");
    if (text) {
      const parsed = parseIngredientString(text);
      ingredients.push({ ...parsed, original: text });
  ...
  const steps: string[] = [];
  $(".recipe-preparation__content p, .recipe-preparation p, .preparation__step").each((_: number, el: any) => {
    const text = $(el).text().trim();
    if (text) steps.push(text);
  });
...
export function parseLdJson(html: string): ScrapedRecipe | null {
  try {
    ...
    const image = Array.isArray(recipeData.image) 
      ...
      : (recipeData.image?.url || recipeData.image || null);
    ...
    const servings = parseInt(String(recipeYield || '').match(/\d+/)?.[0] || '4');
    
    const ingredients: ScrapedIngredient[] = (recipeData.recipeIngredient || []).map((text: string) => ({
       ...parseIngredientString(text),
       original: text
    }));

    const instructions = Array.isArray(recipeData.recipeInstructions) 
      ? recipeData.recipeInstructions.map((s: any) => {
